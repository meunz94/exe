...
| DB (config 키) | 속성 | 출력 |
|---|---|---|
| 게시글 `posts` | 제목(제목), 번호(숫자), 날짜(날짜), 카테고리(선택), 게시판(선택), 미리보기(텍스트), 대표이미지(파일), 공개 + **본문 = 페이지 내용** | `public/data/posts/<카테고리>/<게시판>/<번호>.md` |
| AU 게시글 `auPosts` | 제목, 번호, 날짜, AU(선택: au-1 등), 미리보기, 공개 + 본문 | `public/data/au-posts/<AU>/<번호>.md` |
...
| 플레이리스트 `playlist` | 제목, 아티스트, 길이, 카테고리, 가사(텍스트), 정렬, 공개 | `playlist.json` |
| 갤러리 `gallery` | 이름(제목), 이미지(파일), 섹션(선택), 캡션(텍스트), 공개 | `public/images/gallery/<섹션>/` (섹션이 `au-*` 면 `au/<섹션>/`) |
| 짧은 공지 `notices` | 내용(제목), 카테고리, 정렬, 공개 | `notion.json` → 사이트의 notices |
| 징계 기록 `disciplinary` | 대상(제목), 사유, 날짜, 등급(선택), 카테고리, 공개 | `notion.json` |
| 유튜브 `youtube` | 제목, 영상ID(텍스트), 카테고리, 정렬, 공개 | `notion.json` |
...
- **정렬**: 타임라인·징계는 날짜순, 나머지 목록은 `정렬` 숫자 오름차순.
  게시글 번호(`번호`)는 게시판 안에서 유일해야 하며 파일명이 된다.
...
- 이관이 끝나 노션이 원본이 된 뒤에는 `public/data/posts/template.md` 만 남고
  기존 수기 파일들은 동기화가 정리한다.
